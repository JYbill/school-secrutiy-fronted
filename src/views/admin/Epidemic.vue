...
// 修改文章
const updateItemFunc = (item: Security) => {
...
const deleteItemFunc = (id: number) => {
  dialog.warning({
    ...
    negativeText: '否',
    onPositiveClick: () => {
      EpidemicHttp.deleteEpidemic(id).then(res => {
...
const submitCallback = () => {
  EpidemicHttp.insertEpidemic(itemData.value).then(res => {
...
const submitUpdateCallback = () => {
  EpidemicHttp.updateEpidemic(itemData.value).then(res => {
    message.info('更新成功');
    httpFindAllData();
  })
  itemData.value = {
