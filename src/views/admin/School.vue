...
// 修改文章
const updateItemFunc = (item: School) => {
...
const deleteItemFunc = (id: number) => {
  dialog.warning({
    ...
    negativeText: '否',
    onPositiveClick: () => {
      SchoolHttp.deleteSchool(id).then(res => {
...
const submitCallback = () => {
  SchoolHttp.insertSchool(itemData.value).then(res => {
...
const submitUpdateCallback = () => {
  SchoolHttp.updateSchool(itemData.value).then(res => {
    message.info('更新成功');
    httpFindAllData();
  })
  itemData.value = {
